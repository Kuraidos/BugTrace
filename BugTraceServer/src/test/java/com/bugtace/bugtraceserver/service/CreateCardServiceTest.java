...
class CreateCardServiceTest {
    ...
    void createInProgressCard()
    {
        ...
        Team testTeam= teamRepository.findById(testUser.getTeamIds().get(0)).orElse(null);
        if(testTeam!=null)
        {
            underTest.CreateCard(email,password,testUser.getTeamIds().get(0).toString(),username,title,assignTo,priority.toString(),keywords,description);
    ...
    void createToDOCard()
    {
        ...
        if(testTeam!=null)
        {
            underTest.CreateCard(email,password,testUser.getTeamIds().get(0).toString(),username,title,"",priority.toString(),keywords,description);
            testTeam= teamRepository.findById(testUser.getTeamIds().get(0)).orElse(null);
    ...
    void failCreateCard()
    {
        ...
        if(testTeam!=null)
        {
            underTest.CreateCard(email,email,testUser.getTeamIds().get(0).toString(),username,title,"",priority.toString(),keywords,description);
            testTeam= teamRepository.findById(testUser.getTeamIds().get(0)).orElse(null);
            List<Card> testCards = testTeam.getToDos();
            if(testCards.size()>1){
                Card testCard =testCards.get(0);
